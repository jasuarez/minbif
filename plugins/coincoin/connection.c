...
static guchar *http_gunzip(const guchar *gzip_data, ssize_t *len_ptr)
{
	...
	purple_debug_info("coincoin", "gzip len: %ld, len: %ld\n", gzip_len,
			gzip_data_len);
	purple_debug_info("coincoin", "gzip flags: %d\n", flags);
	purple_debug_info("coincoin", "gzip error: %d\n", gzip_err);
...
static void http_update_cookies(CoinCoinAccount *fba, const gchar *headers)
{
			...
			(headers-cookie_start) < header_len)
	{
		...
		purple_debug_info("coincoin", "got cookie %s=%s\n",
...
static void http_connection_process_data(HttpConnection *fbconn)
{
	...
	if (tmp == NULL) {
		...
		purple_debug_misc("coincoin", "response headers\n%s\n",
				fbconn->rx_buf);
...
static void http_fatal_connection_cb(HttpConnection *fbconn)
{
	...
	purple_debug_error("coincoin", "fatal connection error\n");
		...
		const gchar *error_message)
{
	...
	if (error_message)
	{
		purple_debug_error("coincoin", "post_or_get_connect_cb %s\n",
				error_message);
		http_fatal_connection_cb(fbconn);
		return;
	}

	purple_debug_info("coincoin", "post_or_get_connect_cb\n");
	fbconn->fd = source;
		...
		PurpleSslConnection *ssl, PurpleInputCondition cond)
{
	...
	purple_debug_info("coincoin", "post_or_get_ssl_connect_cb\n");
		...
		const char *error_message)
{
	...
	purple_debug_info("coincoin", "updating cache of dns addresses\n");
	...
	purple_debug_info("coincoin", "Host %s has IP %s\n",
			hostname, ip_address);
 ...
 */
static gchar *http_cookies_to_string(CoinCoinAccount *fba)
{
	GString *str;
		...
		gboolean keepalive)
{
	...
	user_agent = purple_account_get_string(fba->account, "user-agent", "libpurple (plugin coincoin)");
	...
	purple_debug_misc("coincoin", "sending request headers:\n%s\n",
			request->str);
	...
	if (method == HTTP_METHOD_POST)
		purple_debug_misc("coincoin", "sending request data:\n%s\n",
			postdata);
...
{
	CoinCoinAccount *fba = fbconn->fba;
	...
	if (time(NULL) - fbconn->request_time > 5) {
		...
		purple_debug_error("coincoin", "could not connect after retries\n");
		http_fatal_connection_cb(fbconn);
		return;
	}

	purple_debug_info("coincoin", "making connection attempt\n");
